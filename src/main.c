...
int main(int argc, char *argv[]) {
    ...
    /* Logical size locks the game to 640×480 (4:3).  SDL will pillarbox/
       letterbox automatically when the window is a different aspect ratio,
       including fullscreen-desktop on widescreen monitors. */
    SDL_RenderSetLogicalSize(plat.renderer, 640, 480);
    SDL_SetHint(SDL_HINT_RENDER_SCALE_QUALITY, "1"); /* linear filtering for smooth upscale */
    ...
    while (running) 
    {
        ...
        /* F11: toggle fullscreen-desktop (preserves 4:3 via logical size) */
        ...
        switch (state) {
            ...
            case STATE_OVERWORLD: {
                ...
                cam.target_x = player_cx;
                cam.target_y = player_cy;
                camera_update(&cam, dt, 10.0f); /* 10 = snappy but smooth */
